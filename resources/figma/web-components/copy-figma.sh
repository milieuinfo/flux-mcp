...
# de templates relatief importeren, komen in helpers/; de imports worden daarop herschreven.
...
# helpers/, en elke relatieve import wordt herschreven naar '../helpers/<naam>'.
HELPERS=()
for template in "${TEMPLATES[@]}"; do
    ...
    while IFS= read -r spec; do
        ...
        name="$(basename "${spec}")"
        if [ ! -e "${TARGET}/helpers/${name}.ts" ]; then
            mkdir -p "${TARGET}/helpers"
            cp "${helper}" "${TARGET}/helpers/${name}.ts"
            HELPERS+=("${helper#${SOURCE}/}")
        fi
        sed -i.bak "s|from '${spec}'|from '../helpers/${name}'|" "${copied}"
        rm "${copied}.bak"
